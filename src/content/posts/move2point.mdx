...
* LemLib is infinitely configurable, so it supports a large number of customizable parameters for the move to point implementation:
  ...
  *Background: [andrew](https://github.com/meisZWFLZ/) is a LemLib maintainer. The diagram he sent in this screenshot is pretty helpful for understanding exit conditions, I recommend taking a look!*
...
* `prevLateralOut` and `prevAngularOut` simple store the previous lateral and angular PID outputs. `lastPose` similarly stores the last pose the robot was at.
