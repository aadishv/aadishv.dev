...
export default function IsometricGrid() {
  ...
  const calculateGridDimensions = useCallback(() => {
    ...
    const effectiveGridWidth = cols * TRIANGLE_SIDE + (rows % 2 === 1 ? TRIANGLE_SIDE / 2 : 0);
  ...
  const drawGrid = useCallback(() => {
    ...
    for (let r = 0; r < numRows; r++) {
      for (let c = 0; c < numCols; c++) {
        ...
        let rhombus_base_x = c * TRIANGLE_SIDE + (r % 2 === 1 ? TRIANGLE_SIDE / 2 : 0);
        ...
        const up_v1 = { x: rhombus_base_x + TRIANGLE_SIDE / 2, y: rhombus_base_y };
        const up_v2 = { x: rhombus_base_x, y: rhombus_base_y + TRIANGLE_HEIGHT };
        const up_v3 = { x: rhombus_base_x + TRIANGLE_SIDE, y: rhombus_base_y + TRIANGLE_HEIGHT };

        // Down triangle vertices
        const down_v1 = { x: rhombus_base_x + TRIANGLE_SIDE / 2, y: rhombus_base_y + RHOMBUS_HEIGHT };
        const down_v2 = { x: rhombus_base_x, y: rhombus_base_y + TRIANGLE_HEIGHT };
        const down_v3 = { x: rhombus_base_x + TRIANGLE_SIDE, y: rhombus_base_y + TRIANGLE_HEIGHT };
  ...
  const getClickedTriangle = (event: React.MouseEvent<HTMLCanvasElement>): ClickedTriangle | null => {
    const canvas = canvasRef.current;
    ...
    for (let r = 0; r < numRows; r++) {
      for (let c = 0; c < numCols; c++) {
        let rhombus_base_x = c * TRIANGLE_SIDE + (r % 2 === 1 ? TRIANGLE_SIDE / 2 : 0) + xOffset;
        ...
        if (pointInTriangle(clickX, clickY, up_v1.x, up_v1.y, up_v2.x, up_v2.y, up_v3.x, up_v3.y)) {
          return { r, c, type: 'up' };
        }
        if (pointInTriangle(clickX, clickY, down_v1.x, down_v1.y, down_v2.x, down_v2.y, down_v3.x, down_v3.y)) {
  ...
  const saveGrid = () => {
    const gridName = prompt("Enter a name for your grid (optional):") || `Grid-${new Date().toLocaleString()}`;
    ...
    try {
      ...
      localStorage.setItem(`isometricGrid_${gridName}`, JSON.stringify(saveData));
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-5">
      <h1 className="text-3xl font-bold text-gray-800 mb-6">Isometric Grid Paper</h1>

      <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4 mb-6">
        <Label htmlFor="colorPicker" className="text-lg text-gray-700">Choose Color:</Label>
        <input
          type="color"
          id="colorPicker"
          value={currentColor}
        ...
        <Button variant="destructive" onClick={clearGrid}>Clear Grid</Button>
        <Button variant="secondary" onClick={saveGrid}>Save Grid</Button>
      </div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {Object.keys(savedGrids).length === 0 ? (
                <p className="text-gray-500 italic">No grids saved yet. Click "Save Grid" to begin!</p>
              ) : (
                Object.entries(savedGrids).map(([name]) => (
                  <div key={name} className="p-4 bg-gray-50 rounded-lg shadow-sm border border-gray-200">
                    <h3 className="font-semibold text-sm mb-2 truncate">{name}</h3>
                    <div className="flex gap-2">
                      <Button size="sm" onClick={() => loadGrid(name)}>Load</Button>
                      <Button
                        variant="destructive"
                        size="sm"
